...
async function uploadToCdn(url, buffer, preferredFilename) {
  const filenames = preferredFilename ? [preferredFilename] : ['voice.wav', 'voice.mp3'];
  let lastStatus = 0;
  let lastBody = '';
...
async function uploadFile(buffer, filename = 'tts.wav') {
  if (!buffer?.length) {
    const err = new Error('MAX_UPLOAD_EMPTY');
    err.source = 'max-upload';
    throw err;
  ...
  const endpoint = await request('POST', '/uploads', {
    params: { type: 'file' },
  });

  const { url, token: preToken } = endpoint || {};
  if (!url) {
    const err = new Error('MAX_UPLOAD_NO_URL');
    err.source = 'max-upload';
    throw err;
  }

  const uploadData = await uploadToCdn(url, buffer, filename);
...
async function sendAudioAsFile(target, buffer, filename = 'tts.wav', caption = '⬇️ Скачать аудио файлом') {
  const token = await uploadFile(buffer, filename);
  const attachment = { type: 'file', payload: { token } };
  const { sendMessage } = require('./client');
...
module.exports = { uploadAudio, uploadFile, sendAudio, sendAudioAsFile };
