...
const { GEMINI_API_URL, GEMINI_MODEL, DEFAULTS } = require('../config');
...
async function synthesize(text, settings) {
  ...
  try {
    ...
    if (err.status !== 500 && err.code !== 'GEMINI_NO_AUDIO') throw err;
    console.warn('[gemini-tts] retry after', err.status || err.code);
